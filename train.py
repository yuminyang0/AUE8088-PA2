...
def train(hyp, opt, device, callbacks):
    ...
    callbacks.run("on_pretrain_routine_start")
     ...
     # 새로운 앵커 생성 및 설정
    dataset_path = 'datasets/kaist-rgbt/train.txt'
    new_anchors = kmean_anchors(path=dataset_path, n=9, img_size=640, thresh=0.25, gen=1000, verbose=True)
    print(f'New anchors: {new_anchors}')
    model.anchors = torch.tensor(new_anchors).float().view(3, -1, 2)
...
def parse_opt(known=False):
    ...
    # 추가
    parser.add_argument("--rgbt", action="store_true", help="Use RGBT data")  # --rgbt 인수 추가
