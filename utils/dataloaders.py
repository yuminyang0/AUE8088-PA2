...
class LoadRGBTImagesAndLabels(LoadImagesAndLabels):
    ...
    def __getitem__(self, index):
        ...
        mosaic = self.mosaic and random.random() < hyp["mosaic"]
    def load_mosaic(self, index):
        # 네 개의 이미지를 불러와 결합하는 로직 구현
        # 모자이크 이미지를 생성하기 위해 네 개의 무작위 이미지를 선택합니다.
        # 각 이미지를 로드하고 결합하여 하나의 큰 이미지를 생성합니다.
        labels4 = []
        s = self.img_size
        yc, xc = [int(random.uniform(-x, 2 * s + x)) for x in self.mosaic_border]  # mosaic center x, y
        indices = [index] + random.choices(self.indices, k=3)  # 3 additional image indices
        ...
        for i, index in enumerate(indices):
            ...
            # Place img in img4
            if i == 0:  # top left
                img4 = np.full((2 * s, 2 * s, img.shape[2]), 114, dtype=np.uint8)  # base image with padding
                # Location of image in mosaic
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # large image
                x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # small image
            # ... (다른 이미지들도 같은 방식으로 결합)
            img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # paste
            # Labels
...
for ii, (img, (h0, w0), (h, w)) in enumerate(zip(imgs, hw0s, hw1s)):
    ...
    labels = self.labels[index].copy()
    if labels.size:  # normalized xywh to pixel xyxy format
        labels[:, 1:3] += labels[:, 3:5] / 2.0  # (x_center, y_center) -> (x_lefttop, y_lefttop)
        labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
    ...
    if self.augment:
        ...
        nl = len(labels)  # update after albumentations
        # Some augmentations
        im = self.random_perspective(img, labels, degrees=self.hyp['degrees'], translate=self.hyp['translate'], scale=self.hyp['scale'], shear=self.hyp['shear'])
        if isinstance(im, list):
            im = [self.random_perspective(img, labels, degrees=self.hyp['degrees'], translate=self.hyp['translate'], scale=self.hyp['scale'], shear=self.hyp['shear'])[0] for img in im]
        # HSV color-space
        augment_hsv(img, hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"])
